// #include <Arduino.h>
// #include <solarModbus_v1.hpp>
// #include <solarModbus_setup.hpp>

// SolarModbus_v1::SolarModbus_v1(Stream &serial) {
...
// String SolarModbus_v1::exceptionDescription(uint8_t exception) {
...
// // String SolarModbus_v1::exceptionHandler(uint8_t result, String mode, String address) {
// //     if (result != 0) {
...
// void SolarModbus_v1::exceptionHandler(uint8_t result, String mode, String address) {
//     if (result != 0) {
//         #ifdef DEBUG
...
// uint8_t SolarModbus_v1::writeMultipleRegisters(uint16_t *data, uint16_t address, uint8_t nReg) {
//     uint8_t result = 255;
//     int iter = 0;
...
// uint8_t SolarModbus_v1::writeSingleCoil(uint16_t data, uint16_t address) {
//     uint8_t result = 255;
//     int iter = 0;
//     while (result != 0 and iter < nIter) {
...
// uint8_t SolarModbus_v1::readCoils(uint16_t address, void *variable) {
//     uint8_t result = 255;
//     int iter = 0;
//     while (result != 0 and iter < nIter) {
...
// uint8_t SolarModbus_v1::readHoldingRegisters(uint16_t address, void *variable, uint8_t nReg) {
//     uint8_t result = 255;
//     int iter = 0;
//     while (result != 0 and iter < nIter) {
...
// uint8_t SolarModbus_v1::readInputRegisters(uint16_t address, void *variable, uint8_t nReg, bool oneHundredTimes) {
//     uint8_t result = 255;
//     int iter = 0;
//     while (result != 0 and iter < nIter) {
...
// /* Modbus function 0x04 */
// uint8_t SolarModbus_v1::readSingleInputRegisters(uint16_t address, void *variable, bool oneHundredTimes) {
//     uint8_t result = 255;
//     int iter = 0;
//     while (result != 0 and iter < nIter) {
...
// void SolarModbus_v1::setPin() {
//     digitalWrite(PIN_RE_DE, HIGH);
// }

// void SolarModbus_v1::resetPin() {
//     digitalWrite(PIN_RE_DE, LOW);
// }